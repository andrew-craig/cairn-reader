...
**Rule**: When picking up work or browsing tasks, ALWAYS run `task ready` (or `task list`, `task show`) first. Never use Glob/Read/Grep to browse `.chalk/tasks/` directly. The `task` tool gives structured, filtered, priority-sorted output that is faster and more useful than raw file reads. The CLI is called `task` (at `.chalk/scripts/task`), not `tsk`.
