...
---
Phase 4 of feature_90a5. Status, favorite, scroll_position and archive (DELETE) writes go store-first and enqueue an outbox row when the request fails with a NetworkError. A sync worker drains the outbox on reconnect and app foreground, in order, coalescing scroll_position per article and treating 404 on a replayed DELETE as success. Fixes the swallowed archive error from task_179f. Add-URL stays online-only.
