...
- [x] `pkg/auth/validator.go`: add an `onSignatureFailure func()` hook field on `Validator`, `SetOnSignatureFailure`, and refactor `ValidateToken` to retry once via the hook when it hits `ErrInvalidSignature` (extract the existing body into `parseAndValidate`). No change to any other error path or to any exported signature.
- [x] `pkg/auth/key_refresher.go` (new): `PublicKeyFetcher` interface (matches `*VaultClient.GetPublicKey`'s signature so `*VaultClient` satisfies it with no adapter), `KeyRefresher` type wrapping a fetcher + key path + `*Validator`. `NewKeyRefresher` wires itself as the validator's on-signature-failure hook (debounced, to stop crafted-bad-signature spam from hammering Vault). `Start(ctx, interval)` polls on a ticker and pushes `UpdatePublicKey`.
- [x] `pkg/auth/key_refresher_test.go` (new): fake `PublicKeyFetcher`; reuse the existing `generateTestKeys`/`generateTestToken` helpers in `validator_test.go` (same package). Cases: (1) on-demand recovery — rotate the fake fetcher's key, validate a token signed with the new key with *no* `Start()` call, assert it succeeds on the very first call (proves "no restart needed"); (2) periodic poll recovery with a short interval; (3) debounce — two rapid signature failures trigger only one fetch; (4) pre-rotation tokens still validate (no regression to the happy path). Prove case (1) fails without the fix by temporarily no-op'ing the refresh call, run, confirm the expected failure, then restore the real implementation and confirm it passes.
- [x] Add `JWT_PUBLIC_KEY_REFRESH_INTERVAL` to each of the three services' config (default well under the 24h rotation interval, value + justification in the PR): `services/read/content/internal/config/config.go` (`VaultConfig`), `services/explore/recommender/internal/config/config.go` (`VaultConfig`), `services/read/email/internal/config/config.go` (`AuthConfig`).
- [x] Wire `auth.NewKeyRefresher(...).Start(ctx, interval)` into each `main.go` right after its validator/middleware is constructed: `services/read/content/cmd/content/main.go`, `services/explore/recommender/cmd/explore_recommender/main.go`, `services/read/email/cmd/email_ingest/main.go`. No router or handler changes.
- [x] Verify per module (pkg/auth, services/read, services/explore, services/read/email): `gofmt -l . && go vet ./... && go test -race -count=1 ./...`. `golangci-lint` is not installed in this environment (`golangci-lint: command not found`) — could not run it, noted rather than skipped silently.
- [x] Commit only the Go files touched + `.chalk/tasks/task_88aa.md` to the local branch (explicit paths, no `git add -A`). Per final scope instruction: do not push, do not open a PR — the coordinator/user is handling both after review.

## Review (2026-08-09)

**Files changed** (all on branch `fix/p2-c1-jwt-public-key-refresh`, local commit only — not pushed):
- `pkg/auth/validator.go` — added `onSignatureFailure` hook + `SetOnSignatureFailure`; extracted `parseAndValidate`; `ValidateToken` now retries once via the hook on `ErrInvalidSignature`. No other behavior change.
- `pkg/auth/key_refresher.go` (new) — `PublicKeyFetcher` interface + `KeyRefresher`: periodic poll (`Start(ctx, interval)`) and debounced on-demand refresh wired as the validator's signature-failure hook.
- `pkg/auth/key_refresher_test.go` (new) — 4 tests: on-demand recovery with no `Start()` call, periodic-poll recovery, debounce (1 fetch across 2 rapid failures), pre-rotation-token regression guard.
- `services/read/content/internal/config/config.go`, `services/explore/recommender/internal/config/config.go`, `services/read/email/internal/config/config.go` — added `JWT_PUBLIC_KEY_REFRESH_INTERVAL` (default 5m) to each service's Vault/Auth config.
- `services/read/content/cmd/content/main.go`, `services/explore/recommender/cmd/explore_recommender/main.go`, `services/read/email/cmd/email_ingest/main.go` — construct a `KeyRefresher` right after the validator and start it; email-ingest additionally switches from `middleware.NewJWTAuth(publicKey)` to `auth.NewValidator` + `middleware.NewJWTAuthFromValidator` (existing constructor) so main.go holds the `*Validator` the refresher needs. No router/handler changes in any service.

**Poll interval:** `JWT_PUBLIC_KEY_REFRESH_INTERVAL`, default `5m`, same var name across all three services. Justification: well under the 24h rotation default (288 polls/day/service, negligible Vault load); bounds the proactive-recovery window to ≤5 minutes even if the on-demand hook is somehow never triggered; the on-demand hook is the fast path for the common case (recovers on the very next request, no restart).

**Re-fetch-on-signature-failure:** included in this PR, not deferred (`Validator.SetOnSignatureFailure` + `KeyRefresher.refreshOnDemand`, debounced to 10s minimum gap so a client presenting a stale/forged token repeatedly cannot force unbounded Vault reads).

**Failing-test-first proof:** temporarily made `KeyRefresher.refresh()` fetch the key but not call `validator.UpdatePublicKey` (simulating the pre-fix world). Reran `go test -race -run TestKeyRefresher -v ./...`: `TestKeyRefresher_OnDemandRecoversWithoutRestart` and `TestKeyRefresher_PollRecoversProactively` both failed with `invalid token signature` (the exact P2-C1 symptom — not a compile or setup error); `TestKeyRefresher_OnDemandDebounced` and `TestKeyRefresher_PreRotationTokenStillValid` still passed (they don't depend on the fixed line). Restored the real implementation; full `pkg/auth` suite (including the pre-existing tests) passes.

**Verification output per module** — see the PR/report; summary: `gofmt -l` clean, `go vet ./...` clean, `go build ./...` clean, `go test -race -count=1 ./...` green in `pkg/auth`, `services/read`, `services/explore`, `services/read/email`. `golangci-lint` unavailable in this sandbox (not installed) — not run, flagged rather than assumed clean.
